...
def test_offload_blocks_path_traversal_in_session_id(tmp_path, monkeypatch):
    ...
    original_dir = off.REFS_DIR
    off.REFS_DIR = (tmp_path / "offloads" / "refs").resolve()
    off.REFS_DIR.mkdir(parents=True, exist_ok=True)

    # Temporarily lower the offload threshold so the function attempts the
    # write path.  Use monkeypatch so it's restored automatically.
    from app.config import settings
    monkeypatch.setattr(settings, "offload_max_chars", 10)
