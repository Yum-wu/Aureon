NEW_QA_PAIRS = [
    ...
    {"id": "lcf-001", "question": "LangChain v0.2+ 推荐的链式编排语法是什么？", "answer": "LCEL（LangChain Expression Language），使用管道操作符 `|` 连接各组件，支持 streaming、async、并行执行。", "source_article": "langchain-framework-guide"},
    {"id": "lcf-002", "question": "LangChain 中 create_tool_calling_agent 和 create_react_agent 的主要区别是什么？", "answer": "create_tool_calling_agent 依赖模型原生的 function calling 能力，性能更好但需要模型支持；create_react_agent 使用 ReAct prompt 模式，所有模型都兼容但 token 消耗更高。", "source_article": "langchain-framework-guide"},
    {"id": "lcf-003", "question": "LangGraph 相比 LangChain 的核心优势是什么？", "answer": "LangGraph 是有状态 Agent 编排框架，基于有向图的状态机，支持循环和条件分支，内置持久化和人机协作，适合复杂 Agent 工作流和多步骤推理。", "source_article": "langchain-framework-guide"},
