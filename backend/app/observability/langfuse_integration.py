...
Uses the Langfuse ``CallbackHandler`` to automatically trace all
...
async def init_langfuse() -> None:
    ...
    Performs an auth check synchronously and logs the connection status.
    """
    ...
    try:
        ...
        # 启动时检查 Langfuse 连接状态
        try:
            await _handler.check_connection_async()  # type: ignore[union-attr]
            logger.info(
...
async def shutdown_langfuse() -> None:
    ...
    if _handler is None:
        return
    try:
        await _handler.shutdown_async()  # type: ignore[union-attr]
